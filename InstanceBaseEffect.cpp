...
std::map<BYTE, std::string> g_sMemleketNameMap;
#endif
...
void CInstanceBase::RefreshTextTail()
{
	...
	if (IsPC())
	{
		const int8_t memleket = GetMemleket();

		if (memleket == -1)
		{
			CPythonTextTail::Instance().DetachMemleket(GetVirtualID());
		}
		else
		{
			static D3DXCOLOR titlecolor(1.0f, 1.0f, 0.0f, 1.0f);

			std::map<BYTE, std::string>::iterator itor = g_sMemleketNameMap.find(memleket);
			if (g_sMemleketNameMap.end() != itor)
			{
				CPythonTextTail::Instance().AttachMemleket(GetVirtualID(), itor->second.c_str(), titlecolor);
			}
			else
			{
				CPythonTextTail::Instance().DetachMemleket(GetVirtualID());
...
#ifdef ENABLE_MEMLEKET_SYSTEM
void CInstanceBase::RegisterMemleketName(BYTE iIndex, const char* c_szTitleName)
{
	g_sMemleketNameMap.insert(std::make_pair(iIndex, c_szTitleName));
}
#endif
