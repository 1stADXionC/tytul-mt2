...
class GameWindow(ui.ScriptWindow):
	...
	if app.ENABLE_MEMLEKET_SYSTEM:
		def BINARY_ClearMemleketList(self):
			if self.interface.wndMemleket:
				self.interface.wndMemleket.ClearList()

		def BINARY_UpdateMemleketItems(self, idx, vnum, count):
			if self.interface.wndMemleket:
				self.interface.wndMemleket.LoadItemList(idx, vnum, count)

		def BINARY_UpdateMemleketAffs(self, idx, affTye, affVal):
			if self.interface.wndMemleket:
				self.interface.wndMemleket.LoadAffList(idx, affTye, affVal)

		def BINARY_LoadMemleket(self):
			if self.interface.wndMemleket:
				self.interface.wndMemleket.LoadTitleList()
