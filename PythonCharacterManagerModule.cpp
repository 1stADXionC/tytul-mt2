...
PyObject* chrmgrRegisterMemleketName(PyObject* poSelf, PyObject* poArgs)
{
	int	iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BadArgument();
	...
	CInstanceBase::RegisterMemleketName(iIndex, szTitleName);
	return Py_BuildNone();
}

PyObject* chrmgrGetMainCharacterMemleket(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pkInst = CPythonCharacterManager::Instance().GetMainInstancePtr();
	if (!pkInst)
		return Py_BuildValue("i", -1);

	return Py_BuildValue("i", pkInst->GetMemleket());
}
#endif
...
#ifdef ENABLE_MEMLEKET_SYSTEM
		{ "RegisterMemleketName",		chrmgrRegisterMemleketName,				METH_VARARGS },
		{ "GetMainCharacterMemleket",	chrmgrGetMainCharacterMemleket,			METH_VARARGS },
#endif
